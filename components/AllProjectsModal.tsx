...
export const AllProjectsModal: React.FC<AllProjectsModalProps> = ({ projects, onClose, onProjectSelect }) => {
  return (
    <motion.div
      className="fixed inset-0 bg-off-white/50 backdrop-blur-lg z-50 flex items-center justify-center p-4"
    ...
    >
      <motion.div
        className="bg-off-white/90 rounded-2xl shadow-2xl w-full max-w-7xl h-full max-h-[90vh] flex flex-col relative"
        variants={modalVariants}
        exit="exit"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center p-6 border-b border-gray-200 sticky top-0 bg-off-white/80 backdrop-blur-sm z-10 rounded-t-2xl">
